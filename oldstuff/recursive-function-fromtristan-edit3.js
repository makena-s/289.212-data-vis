...
function countFamilies(group, totalcount, runthislist) {
    // there are ways to reapproach and optimise this
    // it's a bit verbose, but relatively easy to adapt/read/understand
    // you can change the if condition to count different things, perhaps controlled by the function params

    result = totalcount;
    
    currentlist = runthislist;
    
    // beetledata["order-Coleoptera"]["suborder-Protocoleoptera"]

    Object.keys(currentlist).forEach(sublist => {
        //console.log('suborder: ' + suborderkey);
        
        if (sublist == group) {
            countFamilies(group, result, currentlist[sublist])
...
document.write(countFamilies('superfamily-Tshekardocoleoidea',0, beetledata["order-Coleoptera"]));
